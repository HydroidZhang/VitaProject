...
参考 Vita Mahjong 三屏结构，图片资源暂用 ColorRect 白模拼接：

### 8.1 主页 `HomeScreen`
- 木纹双开门背景（棕/米色色块）
- 左上头像占位、右上设置按钮
- 居中 Logo「Vita MAHJONG」
- 底部大橙色按钮「关卡 N」
- 「选关」打开关卡列表浮层

### 8.2 游戏页 `GameHUD` + `PlayBackground`
- 深绿游戏区背景
- 顶栏：返回、关卡、分数、匹配数、菜单
- 底栏：洗牌、提示（圆形按钮白模）
- 牌盘居中于顶底栏之间

### 8.3 通关页 `ResultOverlay`
- 半透明遮罩 + 金色标题「才华横溢」
- 三格统计：时间 / 分数 / 连击
- 进度条白模 + 绿色「关卡 N」按钮
...
- `Data/Levels/levels.json` — 3 关配置
- `ProgressManager` 进度存档
- `HomeScreen` / `GameHUD` / `ResultOverlay` 三屏 UI 白模
